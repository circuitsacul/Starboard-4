...
impl Delete {
    pub async fn callback(self, mut ctx: CommandCtx) -> StarboardResult<()> {
        let role = XPRole::delete(&ctx.bot.db, self.xprole.id.get_i64()).await?;
...
impl ClearDeleted {
    pub async fn callback(self, mut ctx: CommandCtx) -> StarboardResult<()> {
        ...
        let xpr = XPRole::list_by_guild(&ctx.bot.db, guild_id_i64).await?;
        ...
        for role in to_delete {
            XPRole::delete(&ctx.bot.db, role.get_i64()).await?;
        }
