...
#[component]
pub fn FullScreenPopup<T, TIV, A, AIV>(title: T, actions: A, children: Children) -> impl IntoView
where
    A: Fn() -> AIV + 'static,
    AIV: IntoView,
    T: Fn() -> TIV + 'static,
    ...
    view! {
        <dialog class="modal modal-open modal-bottom md:modal-middle pt-10 md:p-10">
            <div class="modal-box !h-full !w-full !max-w-4xl !max-h-full flex flex-col px-0">
                ...
                <div class="px-6 flex-1 overflow-y-auto">{children()}</div>
...
#[component]
pub fn Popup<T, TIV, A, AIV>(title: T, actions: A, children: Children) -> impl IntoView
where
    A: Fn() -> AIV + 'static,
    AIV: IntoView,
    T: Fn() -> TIV + 'static,
    ...
    view! {
        <dialog class="modal modal-open modal-bottom md:modal-middle pt-10 md:p-10">
            <div class="modal-box modal-scroll">
                ...
                <div class="py-4 flex-1">{children()}</div>
